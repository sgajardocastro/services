package cl.gesvita.ws.actualizarproyectocodigodeproyecto;
...
/**
 * Servlet implementation class ActualizarProyectoCodigoDeProyecto
 */
@WebServlet("/WSActualizarProyectoCodigoDeProyecto/ActualizarProyectoCodigoDeProyecto")
public class ActualizarProyectoCodigoDeProyecto extends HttpServlet {
    private static final long serialVersionUID = 1L;
    static Logger logger = Logger.getLogger(ActualizarProyectoCodigoDeProyecto.class);

    /**
     * @see HttpServlet#HttpServlet()
     */
    public ActualizarProyectoCodigoDeProyecto() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            stmt = conn.prepareCall("{call PKG_MONITOR_UPDATE.PROYECTO_ASIGNARCODIGOPROYECTO(?,?,?,?,?)}");
                        stmt.setFloat(1, Float.parseFloat(readFieldString(jsonObj,"in_id_proyecto")));
                        stmt.setString(2, readFieldString(jsonObj,"in_modalidad_cambiocodigo"));
                        stmt.registerOutParameter(3, Types.VARCHAR);
                        stmt.registerOutParameter(4, Types.NUMERIC);
                        stmt.registerOutParameter(5, Types.VARCHAR);
                        stmt.execute();
                        dataRetornar.put("out_codi_proyecto", stmt.getString(3));
