package cl.gesvita.ws.actualizarpresupuestocodipartida;
...
/**
 * Servlet implementation class ActualizarPresupuestoCodiPartida
 */
@WebServlet("/WSActualizarPresupuestoCodiPartida/ActualizarPresupuestoCodiPartida")
public class ActualizarPresupuestoCodiPartida extends HttpServlet {
    private static final long serialVersionUID = 1L;
    static Logger logger = Logger.getLogger(ActualizarPresupuestoCodiPartida.class);

    /**
     * @see HttpServlet#HttpServlet()
     */
    public ActualizarPresupuestoCodiPartida() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            stmt = conn.prepareCall("{call PKG_MONITOR_UPDATE.PRESUPUESTO_CODIPARTIDA_UP(?,?,?,?,?)}");
                        clob = conn.createClob();
                        stmt.setFloat(1, Float.parseFloat(readFieldString(jsonObj,"in_id_presupuesto_ccostomov")));
                        stmt.setString(2, readFieldString(jsonObj,"in_codigo_partida"));
                        stmt.setFloat(3, Float.parseFloat(readFieldString(jsonObj,"in_id_usuario_quemodifica")));
                        stmt.registerOutParameter(4, Types.NUMERIC);
                        stmt.registerOutParameter(5, Types.VARCHAR);
                        stmt.execute();
                        dataRetornar.put("out_codigo", stmt.getInt(4));
                        dataRetornar.put("out_mensaje", stmt.getString(5).toString());
                sendData(response, out , dataRetornar);
