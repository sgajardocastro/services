package cl.gesvita.ws.validarproyectocodigo;
...
/**
 * Servlet implementation class ValidarProyectoCodigo
 */
@WebServlet("/WSValidarProyectoCodigo/ValidarProyectoCodigo")
public class ValidarProyectoCodigo extends HttpServlet {
    private static final long serialVersionUID = 1L;
    static Logger logger = Logger.getLogger(ValidarProyectoCodigo.class);

    /**
     * @see HttpServlet#HttpServlet()
     */
    public ValidarProyectoCodigo() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            stmt = conn.prepareCall("{call PKG_MONITOR_UPDATE.VALIDACION_CODIGO(?,?,?,?,?,?,?)}");
                        clob = conn.createClob();
                        stmt.setFloat(1, Float.parseFloat(readFieldString(jsonObj,"in_id_codigo_validacion")));
                        stmt.setString(2, readFieldString(jsonObj,"in_codigo_validacion"));
                        stmt.setFloat(3, Float.parseFloat(readFieldString(jsonObj,"in_id_usuario")));
                        stmt.registerOutParameter(4, Types.NUMERIC);
                        stmt.registerOutParameter(5, Types.NUMERIC);
                        stmt.registerOutParameter(6, Types.NUMERIC);
                        stmt.registerOutParameter(7, Types.VARCHAR);
                        stmt.execute();
                        dataRetornar.put("out_status", stmt.getInt(4));
                        dataRetornar.put("out_intentos_restantes", stmt.getInt(5));
