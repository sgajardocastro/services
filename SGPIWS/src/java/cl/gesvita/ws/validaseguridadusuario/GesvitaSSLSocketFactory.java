...
public class GesvitaSSLSocketFactory extends SSLSocketFactory {
    private SSLSocketFactory socketFactory;
    public GesvitaSSLSocketFactory() {
        try {
          SSLContext ctx = SSLContext.getInstance("TLS");
          ctx.init(null, new TrustManager[]{ new GesvitaTrustManager()}, new SecureRandom());
          socketFactory = ctx.getSocketFactory();
        } catch ( Exception ex ){ throw new IllegalArgumentException(ex); }
    }

      public static SocketFactory getDefault() { return new GesvitaSSLSocketFactory(); }
...
class GesvitaTrustManager implements X509TrustManager {
