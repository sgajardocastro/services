package cl.gesvita.ws.eliminarsolicitudproducto;
...
/**
 * Servlet implementation class EliminarSolicitudProducto
 */
    @WebServlet("/WSEliminarSolicitudProducto/EliminarSolicitudProducto")
public class EliminarSolicitudProducto extends HttpServlet {

    private static final long serialVersionUID = 1L;
    static Logger logger = Logger.getLogger(EliminarSolicitudProducto.class);

    /**
     * @see HttpServlet#HttpServlet()
     */
    public EliminarSolicitudProducto() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            dataRetornar.put("out_codigo", 9999);
            dataRetornar.put("out_mensaje", "Error de parseo JSON");
            sendData(response, out, dataRetornar);
            return;
        }
        try {
            conn = datasource.getConnection();
            stmt = conn.prepareCall("{call PKG_MONITOR_DELETE.SOLICITUD_PRODUCTO_DEL(?,?,?)}");
